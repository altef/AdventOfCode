...
# A quick visualization to figure out where I was going wrong
# Spoiler, I wasn't accounting for wires intersecting with themselves
...
def nodes(nodestr):
	nodes = [{'direction': y[0], 'count': int(y[1:])} for y in nodestr.split(",")]
	position = [0,0]
	while nodes:
		direction = nodes[0]['direction']
		if direction == 'U': position[1] -= 1
		elif direction == 'D': position[1] += 1
		elif direction == 'R': position[0] += 1
		elif direction == 'L': position[0] -= 1
		nodes[0]['count'] -= 1
		if nodes[0]['count'] == 0:
			nodes = nodes[1:]
		yield position
...
def distance(pos):
	print("{}: {}".format(pos, abs(pos[0]) + abs(pos[1])))
	return abs(pos[0]) + abs(pos[1])


def offset(p, d):
	return p*magnification + d/2


wires = iter([{'nodes': nodes(x), 'index': ind} for ind,x in enumerate(wires)])
circuit = {}
intersections = False
...
while True:
	...
	if wire:
		if position:
			...
			circuit[key] = key in circuit;
			# print("{}: {} -> {}".format(wire['index'], previous, position))
			pygame.draw.line(screen, colour, (offset(previous[0], width), offset(previous[1], height)), (offset(position[0], width), offset(position[1], height)), 2)
			pygame.display.flip()
			previous = position[:]
			position = next(wire['nodes'], False)
		else:
			wire = next(wires, False)
			if wire:
				position = next(wire['nodes'])
				colour = [255 if wire['index'] == i else 150 for i in range(0,3)]
				previous = [0,0]
			else:
				intersections = iter([json.loads(x) for x in circuit.keys() if circuit[x]])
	else:
		intersection = next(intersections, False)
		if intersection:
			pygame.draw.circle(screen, (255, 255, 255), (int(offset(intersection[0], width)), int(offset(intersection[1], height))), int(max(magnification/2, 2)), 1)
			pygame.display.flip()
