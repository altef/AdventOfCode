...
# Could have just EVALed, but didn't feel like it
ops = {
	'<': lambda a,b: a < b,
	'>': lambda a,b: a > b,
	'<=': lambda a,b: a <= b,
...
process_max = 0 # I know some values will be higher than this
for line in input.split('\n'):
	reg, op, val, junk, cond_reg, cond_op, cond_val = line.split()
	if reg not in registers:	# Registers start at 0
		registers[reg] = 0
	if cond_reg not in registers:
		registers[cond_reg] = 0
	
	# Validate the condition	if ops[cond_op](registers[cond_reg], int(cond_val)):
		registers[reg] = ops[op](registers[reg], int(val)) # Perform operation
		if registers[reg] > process_max:
			process_max = registers[reg]
