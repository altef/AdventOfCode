...
tree_def = {}
regex = re.compile(r"(\w+)\s\((\d+)\)(\s->\s(.*))?")

# First pass
for line in input.split("\n"):
	match = regex.match(line)
	name, weight, junk, children = match.groups()
	tree_def[name] = {
		'weight': weight,
		'children': None
	}
	if children is not None:
		tree_def[name]['children'] = children.split(', ')
...
def tree(names, depth = 0):
	obj = {}
	for name in names:
		if name not in tree_def: # Some we'll have already processed
			continue
		obj[name] = tree_def[name]
		if tree_def[name]['children'] is not None:
			if type(tree_def[name]['children']) is dict: # Some we'll have already processed
				obj[name]['children'] = tree_def[name]['children']
			else:
				obj[name]['children'] = tree(tree_def[name]['children'], depth + 1)
		if depth > 0:
			del tree_def[name] # Remove any that we've confirmed are children
	return obj


tree = tree(tree_def.keys())
head = tree_def.keys()[0]
print("Part 1: The head is {}".format(head))


# Part 2, recursively calculate weights 
def balance(parent, node, depth = 0):
	if node['children'] is None:
		return 0
	weights = {}
	for name, child in node['children'].iteritems():
		w = int(child['weight']) + balance(name, child, depth+1)
		if w not in weights:
			weights[w] = [0, []]
		weights[w][0] += 1
		weights[w][1].append(name)
		child['total_weight'] = w
	
	# see if there's more than one entry
	if len(weights) > 1:
		# Just output the data, because I want to see it
		print("{}{} is unbalanced.".format("\t"*depth, parent))
		for name, child in node['children'].iteritems():
